...
export function ProfilePage() {
  ...
  const { data, isLoading, error } = useQuery({ queryKey: ['profile'], queryFn: authService.profile, retry: false });
    ...
    <div className="space-y-6">
      ...
      <Card className="grid gap-6 lg:grid-cols-[1.5fr_1fr] p-6">
        ...
        <div className="rounded-3xl border border-slate-800/80 bg-slate-950/80 p-6 text-slate-300">
          ...
          <Button className="mt-6 w-full" onClick={() => navigate('/')}>{t('return_to_dashboard')}</Button>
        </div>
      </Card>
